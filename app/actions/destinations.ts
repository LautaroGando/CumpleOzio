...
export async function adminUpdateDestination(
  id: string,
  data: { name?: string; description?: string; status?: 'LOCKED' | 'UNLOCKED'; comment?: string }
...
/**
 * Admin: update letter content.
 */
export async function adminUpdateLetter(
  content: string,
  signature: string
): Promise<{ success: boolean }> {
  try {
    ...
    if (existing) {
      await prisma.letter.update({ where: { id: existing.id }, data: { content, signature } })
    } else {
      await prisma.letter.create({ data: { content, signature } })
    }
    return { success: true }
  } catch {
    return { success: false }
